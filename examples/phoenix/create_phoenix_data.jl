...
files = Dict(
    "TMYPhoenix_RadPart.mat" => repo_url * "/+data_functions/TMYPhoenix_RadPart.mat",
    "ViewFactor_PH_LCZ3.mat" => repo_url * "/+data_functions/ViewFactor_PH_LCZ3.mat",
)
...
data["location"] = Dict{String,Any}(
    "phi" => 33.4, "lambda" => -122.0, "theta_canyon" => deg2rad(45), "DeltaGMT" => -7.0
)

YAML.write_file(joinpath(data_dir, "phoenix_parameters.yaml"), data)

## NetCDF section
input_data = matread(joinpath(data_dir, "TMYPhoenix_RadPart.mat"))
input_data["Time"] = [
    DateTime(
        input_data["Time"][i, 1],
        input_data["Time"][i, 2],
