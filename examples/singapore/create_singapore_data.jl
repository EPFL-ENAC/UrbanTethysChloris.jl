...
files = Dict(
    "TMYSingapore_RadPart.mat" => repo_url * "/+data_functions/TMYSingapore_RadPart.mat",
    "ViewFactor_SG_LCZ3.mat" => repo_url * "/+data_functions/ViewFactor_SG_LCZ3.mat",
)
...
data["location"] = Dict{String,Any}(
    "phi" => 1.4, "lambda" => 103.9, "theta_canyon" => deg2rad(45), "DeltaGMT" => 8.0
)

YAML.write_file(joinpath(data_dir, "singapore_parameters.yaml"), data)

## NetCDF section
input_data = matread(joinpath(data_dir, "TMYSingapore_RadPart.mat"))
input_data["Time"] = [
    DateTime(
        input_data["Time"][i, 1],
        input_data["Time"][i, 2],
